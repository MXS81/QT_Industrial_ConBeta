...
void TrackWidget::paintEvent(QPaintEvent *event)
{
    ...
    // 4. 面板配色不再自适应 QPalette：
    // 统一从 QSS qproperty 注入；未注入时回退到固定常量（与原暗色风格一致）。
    QColor panelBg   = m_panelBackgroundColor.isValid() ? m_panelBackgroundColor : QColor(55, 65, 81, 220);
    QColor panelLine = m_panelBorderColor.isValid()      ? m_panelBorderColor      : QColor(107, 114, 128);
    QColor titleClr  = m_panelTitleColor.isValid()       ? m_panelTitleColor       : QColor(96, 165, 250);
    QColor textClr   = m_panelTextColor.isValid()        ? m_panelTextColor        : QColor(229, 231, 235);

    painter.setBrush(QBrush(panelBg));
    painter.setPen(QPen(panelLine, 1));
    painter.drawRoundedRect(infoX - padding, infoY - padding, panelWidth, panelHeight, 5, 5);

    // 5. 绘制标题（使用主题强调色）
    painter.setPen(QPen(titleClr));
